...
def render_project_detail(project, data: dict, utilization: dict, person_demand: list):
    ...
    # --- Load activity data (shared across Overview + Activity tabs) ---
    _act_connector = SQLiteConnector(DB_PATH)
    try:
        _comments = _act_connector.get_comments(project.id)
        _audit = _act_connector.get_audit_log(project.id)
    finally:
        _act_connector.close()

    # --- Summary card (always visible above tabs) ---
    _render_summary_card(project)

    # --- Tabbed layout ---
    tab_overview, tab_plan, tab_analysis, tab_activity = st.tabs([
        "Overview", "Plan", "Analysis", "Activity",
    ])

    with tab_overview:
        _render_overview_tab(project, data, _comments, _audit)
    ...
    with tab_activity:
        _render_activity_tab(project.id, project.name, _comments, _audit)


def _render_activity_tab(project_id, project_name, comments, audit_log):
    """Render the full Activity tab — comments, status updates, history."""
    import uuid
    ...
    with st.form(f"comment_form_{project_id}", clear_on_submit=True):
        comment_text = st.text_area("Add a comment", height=80,
                                    ...
                                    key=f"_tab_comment_{project_id}")
    ...
    for a in (audit_log or []):
        ...
        if action in ("comment_added",):
            continue
        icon_map = {
            "status_change": "🔄", "project_created": "✨",
            "project_updated": "📝", "milestone_added": "🎯",
            "milestone_updated": "🎯", "task_added": "✅",
            "task_updated": "✅", "file_uploaded": "📎",
        }
        icon = icon_map.get(action, "📋")
        if a.get("field_changed") and a.get("old_value") and a.get("new_value"):
            body = f"Changed **{a['field_changed']}** from {a['old_value']} to {a['new_value']}"
    ...
    if items:
        for item in items:
            ...
            bg = "#FFF8E1" if item["type"] == "comment" and "status_update" in body else "#FFFFFF"
            st.markdown(
                f'<div style="display:flex; gap:0.75rem; padding:0.65rem 0.5rem;'
                f' border-bottom:1px solid #F0F2F5; background:{bg}; border-radius:4px;'
                f' align-items:flex-start;">'
                f'<div style="width:32px; height:32px; border-radius:50%; background:{NAVY};'
                f' color:#FFF; display:flex; align-items:center; justify-content:center;'
                f' font-size:0.75rem; font-weight:700; flex-shrink:0;">{initial}</div>'
                f'<div style="flex:1; min-width:0;">'
                f'<div style="display:flex; justify-content:space-between; align-items:baseline;">'
                f'<span style="font-size:0.85rem; font-weight:600; color:#2C3E50;">{author}</span>'
                f'<span style="font-size:0.73rem; color:#ADB5BD;">{time_str}</span></div>'
                f'<div style="font-size:0.83rem; color:#5A6A7E; margin-top:0.2rem;">'
                f'{item["icon"]} {body}</div></div></div>',
                unsafe_allow_html=True,
            )
    else:
    ...
    uploaded = st.file_uploader("Upload a file", key=f"_tab_upload_{project_id}",
    ...
    if attachments:
        for att in attachments:
            att_path = Path(__file__).parent / att["stored_path"]
            col1, col2 = st.columns([5, 1])
            with col1:
                size_kb = att["file_size"] / 1024
                st.markdown(
                    f'📎 **{att["filename"]}** ({size_kb:.0f} KB) — '
                    f'uploaded by {att["uploaded_by"]}',
                )
            with col2:
                if att_path.exists():
                    st.download_button("Download", att_path.read_bytes(),
